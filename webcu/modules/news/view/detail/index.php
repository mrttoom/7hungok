<?php
	global $database;
	global $current_lang;
	$query = 'SELECT * from news_category where lang="'.$current_lang.'" and id= "'.getParam('category_id', 'int', 0).'"';
	$database->setQuery($query);
	$database->query();
	$current_news_cat = $database->loadRow();
	if(!$current_news_cat)
	{
		replace_location('news');
		return;
	}
	$query = 'SELECT * from news where lang="'.$current_lang.'" and id= "'.getParam('id', 'int', 0).'"';
	$database->setQuery($query);
	$database->query();
	...
	$query = 'SELECT * FROM `news` WHERE lang="'.$current_lang.'" and category_id="'.$row['category_id'].'" and id <"'.getParam('id', 'int', 0).'" order by id desc limit 0,5';
	$database->setQuery($query);
	...
	if($other_news)
	{
		...
		{			
			$other_news_html .= '<img src="'.SITE_PATH.'themes/images/icon-news.gif" /><a style="padding-left:5px" href="'.generate_url('news_detail', array('category_id'=>$one['category_id'], 'id'=>$one['id'])).'">'.string_strip($one['title']).'&nbsp;<b>('.date('d/m', $one['time_create']).')</b></a><br />';						
		}
	}
?>
<div class="tdr">
	...
	<a href="<?php echo generate_url('news') ?>"><?php echo _NEWS ?></a>
...
<?php 
	$image_file = get_image($row, 'large', 'news');
