<?php
	...
	function listNewsCategory()
	{
		...
		$totalRows = $database->getNumRows('news_category', ' lang="'.$current_lang.'"'.$condition);				
				
		$query = 'SELECT * FROM `news_category` WHERE lang="'.$current_lang.'" '.$condition.' ORDER BY id desc limit '.(($curPg-1)*$itemPerPage).','.$itemPerPage;
		...
		if($action == "save_order")
		{
			...
			replace_location('news_category_admin');
			exit;
		}
	...
	function deleteNewsCategory()
	{
		...
		if(count($list_id)>0)
		{
			foreach($list_id as $id)
			{
				...
				{
					$database->setQuery('delete from news where category_id = "'.$id.'" and lang="'.$current_lang.'"');
					$database->query();
		...
		replace_location('news_category_admin', array('curPg'));
	}
	function addNewsCategory()
	{
		...
		if($action == "save" || $action == "apply")
		{
			...
			if($is_valid)
			{
				...
				$time_create = time();
				$query = "INSERT INTO news_category (`title`, `user_create`, `time_create`, `user_update`, `time_update`, `lang`, `region`)
				...
				if($action == "save")
					...
					replace_location('news_category_admin', array('task'=>'add'));
			}
		}
	...
	function editNewsCategory()
	{
		...
		if($action == "save" || $action == "apply")
		{
			...
			if($is_valid)
			{
				...
				$time_update = time();
				$query = "UPDATE news_category SET `title`='$title', `user_update`='$user_update', `time_update` = '$time_update', `lang`='$current_lang', `region`='$region' WHERE `id`='".$id."'";
