	...
	?>
		<script >
			...
			{	
				...
				if (res) 
				{
					document.location.href= "<?php echo generate_url('news_admin', array('task'=>'delete', 'curPg')); ?>&value="+value;
				}
			}
			...
			{	
				...
				if (res) 
				{
					document.location.href= "<?php echo generate_url('news_admin', array('task'=>'delete_image', 'curPg')); ?>&value="+value;
				}
			}
			function editNews()
			{	
				...
				document.location.href= "<?php echo generate_url('news_admin', array('task'=>'edit', 'curPg')); ?>&id="+value;
			}
						...
						<table id="toolbar" border="0" cellpadding="0" cellspacing="0">
							<tbody>
								<tr align="center" valign="middle">
									<td>
										<a class="toolbar" href="<?php echo generate_url('news_admin', array('task'=>'add', 'curPg')); ?>">
					...
					<tbody>
						<tr>
							...
							<th class="title" width="20%">
								<?php echo _NEWS_CATEGORY;?>: <select name="category_id" onchange="this.form.submit()"><?php echo get_option($news_category_arr, getParam('category_id', 'int', 0)); ?></select>
							</th>
				...
				if($rows)
				{
					...
					foreach($rows as $row)
					{
						...
						<tr class="<?php echo $class_row; ?>" valign="top">
							...
							<td><?php echo $i; ?></td>
							<td>
								<a href="<?php echo generate_url('news_admin', array('task'=>'edit', 'id'=>$row['id'])); ?>"><?php echo $row['title']; ?></a>
								<br />
							...
							<td>
								<?php
									$image_file = get_image($row, 'small', 'news');
									if($image_file)
			...
			<div class="main">
				<table class="adminheading">
					<tbody>
						<tr>
							<th>
								<?php echo _NEWS_ADMIN;?>: <small><?php echo _UPDATE_NEWS;?></small>
							</th>
						</tr>
					</tbody>
				</table>
