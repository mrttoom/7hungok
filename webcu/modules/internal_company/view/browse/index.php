<?php
	...
	$query = 'SELECT * FROM `internal_company_category` where `lang`= "'.$current_lang.'" and id = "'.getParam('category_id', 'id', 0).'"';
...
<div class="tdr">
	...
	<a href="<?php echo generate_url('internal_company') ?>"><?php echo _INTERNAL_COMPANY ?></a><?php if($category_current) echo ' &raquo; '.$category_current['title'];?>
	</div></div>
...
<?php
	if($all_news)
	{
		foreach($all_news as $one)
		{
			...
			$image_src = get_image($one, 'small', 'internal_company');
			if($image_src)
