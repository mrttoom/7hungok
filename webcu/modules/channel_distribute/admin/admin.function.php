<?php
	...
	function deleteNews()
	{
		...
		if(count($list_id)>0)
		{
			foreach($list_id as $id)
			{
				...
				{
					$database->setQuery('select id, image_name from channel_distribute where id = "'.$id.'" and lang="'.$current_lang.'"');
					$row = $database->loadRow();
					delete_image($row, 'channel_distribute');
					$database->setQuery('delete from channel_distribute where id = "'.$id.'" and lang="'.$current_lang.'"');
					$database->query();
				}
			}
		}
		replace_location('channel_distribute_admin', array('curPg'));
	}
		...
		if(count($list_id)>0)
		{
			foreach($list_id as $id)
			{
				if(isset($id) && $id>0 && is_numeric($id))
				{
					...
					$row = $database->loadRow();
					delete_image($row, 'channel_distribute');
					$database->setQuery('update channel_distribute set `image_name`="" where id = "'.$id.'" and lang="'.$current_lang.'"');
					$row = $database->query();
				}
			}
		}
	...
	function addNews()
	{
		...
		if($action == "save" || $action == "apply")
		{
			...
			$image_name = '';
			$database->setQuery('SELECT max(id) as max_id from channel_distribute');
			$row = $database->loadRow();
			$max_id = $row['max_id']+1;
			...
			if(isset($_FILES['imgfile']) and $_FILES['imgfile']['error']==0)
			{
				...
				else
				{
					...
					upload_image(DATA_PATH.'images/channel_distribute/', $_FILES['imgfile']['tmp_name'], $image_name);
				}
			}
			if(!$error_upload)
			{
				$category_id = getParam('category_id', 'int', 0);
				$query = 'SELECT * FROM `channel_distribute_category` WHERE lang="'.$current_lang.'" and id = "'.$category_id.'" ORDER BY id desc';
				$database->setQuery($query);
				$news_category = $database->loadRow();
				$category_name = $news_category['title'];
				$user_create = $_SESSION['user']['user_name'];
				$time_create = time();
				$query = "INSERT INTO channel_distribute (`id`, `title`, `brief`, `content`, `category_id`, `category_name`, `user_create`, `time_create`, `user_update`, `time_update`, `lang`, `image_name`)
				...
				if($action == "save")
					replace_location('channel_distribute_admin');
				else
					replace_location('channel_distribute_admin', array('task'=>'add'));
			}
		}
	...
	function editNews()
	{
		...
		$action = getParam("action", 'str');
		$id = getParam("id", 'int');
		$query = "SELECT * FROM channel_distribute WHERE id = '".$id."' and lang='".$current_lang."'";
		$database->setQuery($query);
		$row = $database->loadRow();
		if(!$row)
		{
			echo '	<script>
						...
						window.location="'.generate_url('channel_distribute_admin', array('curPg')).'";
					</script>';
			return;
		}
		$error_array = array();
		if($action == "save" || $action == "apply")
		{
			...
			if(!$error_upload)
			{
				...
				$query = "UPDATE channel_distribute SET `title`='$title', `brief`='$brief', `content` = '$content', `category_id`='$category_id', `category_name`='$category_name', `image_name`='$image_name', `user_update`='$user_update', `time_update` = '$time_update', `lang`='$current_lang' WHERE `id`='".$id."'";
